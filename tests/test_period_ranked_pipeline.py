...
def test_global_ranking_returns_top_four_only():
    items = [{"title":"A","editorial_score":91},{"title":"B","editorial_score":95},{"title":"C","editorial_score":80},{"title":"D","editorial_score":88},{"title":"E","editorial_score":99}]
...
def test_protected_leader_enters_tier0_ranking():
    items = [{"leader":"Leader A","protected_content":True,"leader_watch_protected":True,"leader_source_authority":3,"editorial_score":80,"title":"Leader interview"},{"editorial_score":100,"title":"Regular top"}]
    ranked = pipeline._global_ranked_selection(items, 1, 2, 2, {})
...
def test_tier0_interview_is_not_blocked_by_semantic_similarity(monkeypatch):
    record = {
        "title": "Jensen Huang discusses AI infrastructure",
        "link": "https://example.com/old-interview",
    }
    candidate = {
        "title": "Jensen Huang on the future of AI computing",
        "link": "https://example.com/new-interview",
        "content_type": "interview",
        "summary": "Jensen Huang gives a substantive interview about AI infrastructure, accelerated computing, agents, robotics and the future of technology.",
    }
    ...
    kept = pipeline._exclude_published_candidates([candidate])
    assert kept == [candidate]


def test_protected_leader_story_bypasses_semantic_history(monkeypatch):
    record = {
        "title": "Dario Amodei discusses AI safety and model development",
    ...
    candidate = {
        ...
        "summary": "A substantive interview about AI safety, frontier models and deployment risks.",
    }
    monkeypatch.setattr(pipeline, "_load_records", lambda: [record])
...
def test_tier0_exact_url_repeat_is_still_blocked(monkeypatch):
    ...
    candidate = {
        "title": "A different title for Jensen Huang",
        "link": "https://example.com/interview",
        "content_type": "interview",
        "summary": "Jensen Huang gives a substantive interview about AI infrastructure and computing.",
    }
    monkeypatch.setattr(pipeline, "_load_records", lambda: [record])
    monkeypatch.setattr(pipeline, "_semantic_conflict", lambda *args, **kwargs: 0.95)
