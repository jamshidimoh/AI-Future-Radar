...
RES={"paper","research","study","preprint","scientific","experiment","findings","method","benchmark","peer reviewed","journal article","پژوهش","مطالعه","آزمایش"}
...
def _topic(t,extra=None):
    ...
    if _has(t,BIO): return ("bio_ai",_hits(t,BIO)[:6]) if _has(t,AI|AI_BRIDGE|{"machine learning","computational"}) else ("out_of_scope",[])
    if _has(t,BCI): return ("bci_neuro_ai",_hits(t,BCI)[:6]) if _has(t,AI|AI_BRIDGE|{"computational","neural"}) else ("out_of_scope",[])
    if _has(t,ROB): return ("robotics_embodied",_hits(t,ROB)[:6]) if _has(t,AI|AI_BRIDGE|{"autonomous","learning"}) else ("out_of_scope",[])
    if _has(t,COMP): return ("computing_infrastructure",_hits(t,COMP)[:6]) if _has(t,AI|AI_BRIDGE|{"inference","training","accelerator"}) else ("out_of_scope",[])
    return ("ai_core",_hits(t,AI|AI_BRIDGE|e)[:6]) if _has(t,AI|AI_BRIDGE|e) else ("out_of_scope",[])
...
def _age(p):
    ...
    try:return max(0,(time.time()-time.mktime(time.strptime(p,"%Y-%m-%d %H:%M")))/3600)
        ...
        logger.warning("Could not parse publication age %s: %s", p, exc, exc_info=True)
        return None
