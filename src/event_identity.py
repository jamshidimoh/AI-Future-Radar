...
The matcher deliberately combines deterministic entity/action signals with
context similarity. Shared company/person/topic alone is not enough to classify
an article as the same story. Material updates are kept distinct from pure
repeats.
"""
...
ENTITIES = {
    ...
    "terence_tao": {"terence tao", "tao", "ترنس تائو", "ترنس تائو"},
...
def entity_set(text: Any) -> set[str]:
    ...
    for canonical in ENTITIES:
        if re.search(r"(?<![\w])" + re.escape(canonical) + r"(?![\w])", s):
            found.add(canonical)
    return found
...
def event_set(text: Any) -> set[str]:
    ...
    for canonical, aliases in EVENTS.items():
        for alias in aliases:
            if re.search(r"(?<![\w])" + re.escape(alias) + r"(?![\w])", s):
                found.add(canonical)
                break
    return found
...
def event_features(item: dict[str, Any]) -> dict[str, Any]:
    ...
    norm = normalize(text)
    return {
        "entities": entity_set(text),
        "events": event_set(text),
        "tokens": tokens(text),
        "material": material_set(text),
        "title": normalize(item.get("title", "")),
        "event_time": _parse_time(item.get("event_time") or item.get("published") or item.get("published_at")),
...
def has_material_update(a: dict[str, Any], b: dict[str, Any]) -> bool:
    ...
    numbers_a = set(re.findall(r"\b\d+(?:\.\d+)?\b", fa["norm"]))
    numbers_b = set(re.findall(r"\b\d+(?:\.\d+)?\b", fb["norm"]))
    if numbers_a and numbers_b and numbers_a != numbers_b:
        return True
    new_material = fa["material"] - fb["material"]
    return len(new_material) >= 2
...
def compare_events(a: dict[str, Any], b: dict[str, Any]) -> tuple[str, float, dict[str, Any]]:
    ...
    time_gap = None
    if fa["event_time"] and fb["event_time"]:
        time_gap = abs((fa["event_time"] - fb["event_time"]).total_seconds()) / 86400.0

    score = 0.0
    score += 0.35 * _jaccard(shared_entities, shared_entities | {"__dummy__"})
    score += 0.20 * (1.0 if shared_events else 0.0)
    score += 0.25 * context
    score += 0.20 * title
    if strong_product:
        score += 0.18
    if strong_person and shared_events:
        score += 0.12
    if strong_source and (shared_events or context >= 0.35):
        score += 0.12
    if time_gap is not None and time_gap > 45:
        score *= 0.65
    ...
    evidence = {
        ...
        "context_jaccard": round(context, 4),
        "title_similarity": round(title, 4),
        "time_gap_days": time_gap,
        "material_update": material,
    }

    # High-confidence exact/event rewrites. Require at least one concrete entity
    # plus event/context agreement; a shared organization alone never qualifies.
    same = False
    if strong_product and (shared_events or context >= 0.22):
        same = True
    elif strong_person and shared_events and context >= 0.18:
        same = True
    elif strong_source and context >= 0.35:
        same = True
    elif len(shared_entities) >= 2 and shared_events and context >= 0.20:
        same = True
    elif title >= 0.90 and context >= 0.45:
        same = True

    if same and material:
        return "UPDATE", min(1.0, score), evidence
    if same:
        return "DUPLICATE", min(1.0, score), evidence
    if shared_entities and (shared_events or context >= 0.16):
        return "RELATED", min(1.0, score), evidence
