...
def _llm_check(source_title: str, source_text: str, draft: dict) -> bool | None:
    prompt = _CHECK_PROMPT.format(
        ...
        draft=json.dumps(draft, ensure_ascii=False)[:5000],
    )
    try:
        raw, _provider = call_llm_with_fallback(
            prompt,
            json.dumps({"source_title": source_title, "source_text": source_text[:5000], "draft": draft}, ensure_ascii=False),
...
def ensure_source_grounding(draft: dict, item: dict) -> dict | None:
    ...
    if repaired is not None:
        ...
        if repaired_verdict is True or repaired_overlap >= _THRESHOLD:
            repaired["source_grounding_score"] = round(repaired_overlap, 3)
            repaired["source_grounding_repaired"] = True
            print(f"[Source Grounding] repaired score={repaired_overlap:.2f}", flush=True)
            return repaired

    print(f"[Source Grounding] blocked overlap={overlap:.2f}", flush=True)
    return None
