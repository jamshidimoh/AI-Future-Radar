...
def _is_community(item: dict[str, Any]) -> bool:
    ...
    if any(m in value for m in _COMMUNITY_MARKERS): return True
...
def _safe_float(item: dict[str, Any], key: str) -> float:
    try: return float(item.get(key, 0) or 0)
...
def _is_research(item: dict[str, Any]) -> bool: return content_type_key(item) in _RESEARCH_TYPES or bool(item.get("research_signal"))
def _is_interview(item: dict[str, Any]) -> bool: return content_type_key(item) in _INTERVIEW_TYPES or bool(item.get("interview_signal"))
def _authority_ok(item: dict[str, Any]) -> bool: return _source_tier(item) in {1, 2}
...
def select_regular_portfolio(candidates: Iterable[dict[str, Any]], *, max_posts: int, max_per_source: int, max_per_type: int, recent_source_counts: dict[str, int] | None = None, contract: dict[str, Any] | None = None, mission_aware: bool = True, strict_relevance: bool = False) -> list[dict[str, Any]]:
    ...
    selected: list[dict[str, Any]] = []; selected_ids: set[int] = set(); source_counts: dict[str, int] = {}; type_counts: dict[str, int] = {}; area_counts: dict[str, int] = {}; interview_count = 0
    ...
    def admissible(item: dict[str, Any], *, repeat_source: bool) -> bool:
        ...
        if type_counts.get(ctype, 0) >= type_cap: return False
        if mission_aware and _is_interview(item) and interview_count >= contract["interview_target_max"] > 0: return False
        if mission_aware and area_counts.get(area, 0) >= contract["max_same_mission_area"]: return False
    ...
    def add(item: dict[str, Any], reason: str) -> None:
        ...
        source, ctype, area = source_key(item), content_type_key(item), mission_area(item)
        selected.append(item); selected_ids.add(id(item)); source_counts[source] = source_counts.get(source, 0) + 1; type_counts[ctype] = type_counts.get(ctype, 0) + 1
        if mission_aware: area_counts[area] = area_counts.get(area, 0) + 1
        if _is_interview(item): interview_count += 1
        item["mission_selection_reason"] = reason

    def remove(item: dict[str, Any]) -> None:
        nonlocal interview_count
        selected.remove(item); selected_ids.remove(id(item)); source_counts[source_key(item)] -= 1; type_counts[content_type_key(item)] -= 1
        if mission_aware: area_counts[mission_area(item)] -= 1
        if _is_interview(item): interview_count -= 1
    ...
    def best(pool: list[dict[str, Any]], *, prefer_research: bool = False) -> dict[str, Any] | None:
        ...
        if not candidates2: return None
        if prefer_research:
            research_first = [x for x in candidates2 if _is_research(x)]
            if research_first: candidates2 = research_first
    ...
    for item in ordered:
        if len(selected) >= limit or id(item) in selected_ids: break
        if admissible(item, repeat_source=False):
            if mission_aware and mission_area(item) == "ai_core" and area_counts.get("ai_core", 0) >= contract["ai_core_target_max"]: continue
            add(item, "score_fill")
    ...
    while mission_aware and sum(_authority_ok(x) for x in selected) < auth_required:
        ...
        if replacement is None or not removable: break
        victim = max(removable, key=lambda x: (_rank_key(x, recent), -candidate_score(x)))
        remove(victim); add(replacement, "policy_repair:min_authoritative_items")
    return selected[:limit]


def assert_portfolio_contract(selected: Iterable[dict[str, Any]], *, contract: dict[str, Any] | None = None) -> None:
    contract = contract or load_editorial_contract(); items = list(selected or []); source_counts: dict[str, int] = {}; area_counts: dict[str, int] = {}
    for item in items:
        source_counts[source_key(item)] = source_counts.get(source_key(item), 0) + 1; area_counts[mission_area(item)] = area_counts.get(mission_area(item), 0) + 1
    ...
    if len(items) >= contract["min_authoritative_items"]: assert sum(1 for item in items if _authority_ok(item)) >= contract["min_authoritative_items"]
