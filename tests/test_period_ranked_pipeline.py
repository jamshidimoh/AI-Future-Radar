...
def test_protected_leader_is_reserved_and_selected():
    ...
    try:
        ...
        assert protected[0]["protected_slot"] is True
...
def test_protected_leader_enters_tier0_ranking_via_eligibility_contract():
    monkeypatch = __import__("pytest").MonkeyPatch()
    try:
        monkeypatch.setattr(pipeline._pipeline, "_is_protected_leader_interview", lambda item: item.get("leader") == "Leader A")
        monkeypatch.setattr(pipeline._pipeline, "_is_protected_leader_activity", lambda item: False)
        ...
        protected, regular = pipeline._eligibility_split(
            [
                {"leader":"Leader A","editorial_score":80,"title":"Leader interview"},
                {"editorial_score":100,"title":"Regular top","source":"source-regular","category":"ai"},
            ],
            2,
        )
        ranked = pipeline._global_ranked_selection(protected + regular, 1, 2, 2, {})
        assert ranked[0]["title"] == "Leader interview"
        assert ranked[0]["protected_slot"] is True
...
def test_tier0_interview_is_not_blocked_by_semantic_similarity(monkeypatch):
    ...
    candidate = {"title": "Jensen Huang on the future of AI computing", "link": "https://example.com/new-interview", "content_type": "interview", "protected_slot": True, "summary": "Jensen Huang gives a substantive interview about AI infrastructure, accelerated computing, agents, robotics and the future of technology."}
...
def test_protected_leader_story_bypasses_semantic_history(monkeypatch):
    ...
    candidate = {"title": "Dario Amodei on the next phase of AI safety", "link": "https://example.com/new", "content_type": "interview", "protected_content": True, "protected_slot": True, "leader_watch_protected": True, "summary": "A substantive interview about AI safety, frontier models and deployment risks."}
    monkeypatch.setattr(pipeline, "_load_records", lambda: [record])
...
def test_tier0_exact_url_repeat_is_still_blocked(monkeypatch):
    ...
    candidate = {"title": "A different title for Jensen Huang", "link": "https://example.com/interview", "content_type": "interview", "protected_slot": True, "summary": "Jensen Huang gives a substantive interview about AI infrastructure and computing."}
    monkeypatch.setattr(pipeline, "_load_records", lambda: [record])
    monkeypatch.setattr(pipeline, "_semantic_conflict", lambda *args, **kwargs: 0.95)
