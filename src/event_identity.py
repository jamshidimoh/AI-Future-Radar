...
def has_material_update(a:dict[str,Any],b:dict[str,Any])->bool:
    ...
    return (bool(na and nb and na!=nb) or len(fa["material"]-fb["material"])>=2)
...
def compare_events(a:dict[str,Any],b:dict[str,Any])->tuple[str,float,dict[str,Any]]:
    ...
    same = (
        (strong_product and bool(shared_events)) or
        (strong_product and context>=0.55 and title>=0.65) or
        (strong_person and bool(shared_events) and context>=0.18) or
        (strong_source and context>=0.35) or
        (bool(shared_events) and context>=0.65) or
        (len(shared)>=2 and bool(shared_events) and context>=0.20) or
        (title>=0.90 and context>=0.45)
    )
    ...
    if same and material_update: return "UPDATE",score,{"shared_entities":sorted(shared),"shared_events":sorted(shared_events),"context_jaccard":round(context,4),"title_similarity":round(title,4),"material_update":True}
    if same: return "DUPLICATE",score,{"shared_entities":sorted(shared),"shared_events":sorted(shared_events),"context_jaccard":round(context,4),"title_similarity":round(title,4),"material_update":False}
    if shared and (shared_events or context>=0.16): return "RELATED",score,{"shared_entities":sorted(shared),"shared_events":sorted(shared_events),"context_jaccard":round(context,4),"title_similarity":round(title,4),"material_update":material_update}
    return "NEW",score,{"shared_entities":sorted(shared),"shared_events":sorted(shared_events),"context_jaccard":round(context,4),"title_similarity":round(title,4),"material_update":material_update}
