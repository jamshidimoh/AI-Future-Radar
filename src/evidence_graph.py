...
class EvidenceGraph:
    ...
    def _by_kind(self, kind: EvidenceKind, claim_id: str | None) -> List[Evidence]:
        return [e for e in self.evidences.values() if e.kind == kind and (claim_id is None or e.claim_id == claim_id)]

    def independent_source_count(self, claim_id: str | None = None) -> int:
        rows = [e for e in self.evidences.values() if claim_id is None or e.claim_id == claim_id]
