...
youtube = ROOT / "src" / "fetch_youtube.py"
replace_once(
    youtube,
...
while text.count(helper) > 1:
    text = text.replace(helper + helper, helper, 1)
if text != summarize.read_text(encoding="utf-8"):
...
# Ensure both repair paths use the strongest evidence context. These replacements
# are idempotent because the runner is also used after partial application.
replace_all(
    summarize,
