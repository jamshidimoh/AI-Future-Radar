...
def test_protected_tier0_uses_independent_absolute_quality_floor():
    assert PROTECTED_SCORE_FLOOR == 55.0
    assert protected_score_allowed(55.0)
    assert protected_score_allowed(75.0)
    assert not protected_score_allowed(54.99)
...
[Publication Contract] candidate rejected reason=tier0_score_policy_blocked:28.16<floor:55.0; continuing to next ranked candidate
[Production Contract] normal_news=0 normal_max=3 tier0_news=0 tier0_quota_exempt=true tier0_quality_floor=55.0 education=not_due
Posts sent: 0/1
"""
...
Posts sent: 1/1
[Production Contract] normal_news=0 normal_max=3 tier0_news=1 tier0_quota_exempt=true tier0_quality_floor=55.0 education=not_due
"""
    ok, reason = validate(log)
    assert ok is False
