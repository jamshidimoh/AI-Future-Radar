...
def _chatgpt_link(title, link):
    prompt = ("این خبر را از منبع اصلی بررسی کن. اعتبار منبع، ادعاهای کلیدی، شواهد، نکات موافق و مخالف "
              "و اهمیت آن برای آینده فناوری را تحلیل کن و پاسخ را به فارسی بده.\n\n"
              f"عنوان: {title}\nمنبع: {link}")
