...
def test_production_uses_canonical_router_module_and_trust_order(monkeypatch):
    ...
    assert names[0] == "OpenRouter:nvidia/nemotron-3-ultra-550b-a55b:free"
    assert names[1] == "OpenRouter:nvidia/nemotron-3-super-120b-a12b:free"
    assert names[2] == "Groq:openai/gpt-oss-120b"
    assert "Groq:qwen/qwen3.8-27b" not in names
...
def test_production_quota_is_model_scoped_and_sibling_can_failover(monkeypatch):
    ...
    providers = [
        ("Groq:openai/gpt-oss-120b", first_fails),
        ("Groq:qwen/qwen3.6-27b", sibling_ok),
    ]
