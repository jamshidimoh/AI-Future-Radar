...
patch(
    ...
    r"from src\.protected_editorial_lane import SPECIAL_MAX_PER_PERIOD, choose_additive_candidates\n",
...
patch(
    "production_entrypoint.py",
    r"MAX_MIND_IDEAS_VOICES_PER_PERIOD = SPECIAL_MAX_PER_PERIOD\n",
...
patch(
    "production_entrypoint.py",
    r"for item in protected \+ normals \+ mind:",
    "for item in protected + normals + technical + mind:",
    flags=0,
)
patch(
    "production_entrypoint.py",
    r"normal=\{len\(normals\)\} protected=\{len\(protected\)\} mind_ideas_voices=\{len\(mind\)\} output=\{len\(bounded\)\}",
...
s = p.read_text(encoding="utf-8")
if old not in s:
    raise SystemExit("PATCH_FAILED selection block")
